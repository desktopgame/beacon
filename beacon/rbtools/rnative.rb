...
File.open("rnative_h.text", "w") do |file|
	...
	file.puts
	file.puts(sprintf("void bc_%s_init();", typename.downcase()));
	file.puts
	file.puts(sprintf("struct type* bc_%s_type();", typename.downcase()));
	file.puts
	file.puts("#endif")
end
File.open("rnative_c.text", "w") do |file|
	...
	method_name_list.each do|name|
		file.puts(sprintf("static void bc_%s_native%s(method* parent, frame* fr, enviroment* env);", typename.downcase, name.capitalize));
	end
	file.puts
	file.puts(sprintf("void bc_%s_init() {", typename.downcase()));
	file.puts("}")
	file.puts
	file.puts(sprintf("type* bc_%s_type() {", typename.downcase()));
