...
bc_NativeThread* bc_AllocNativeThread(bc_Runnable runnable,
        ...
        bc_NativeThread* ret =
            (bc_NativeThread*)MEM_MALLOC(sizeof(bc_NativeThread));
...
bc_NativeThread* bc_GetMainThread() { return bc_GetNativeThreadAt(0); }
...
bc_NativeThread* bc_GetActiveThread() {
        ...
        for (int i = 0; i < gThreads->Length; i++) {
                bc_NativeThread* th =
                    (bc_NativeThread*)bc_AtVector(gThreads, i);
...
int bc_GetNativeThreadCount() { return gThreads->Length; }

void bc_DestroyNativeThread() {
        assert(gThreads != NULL);
        while (gThreads->Length > 0) {
                bc_NativeThread* nt =
                    (bc_NativeThread*)bc_AtVector(gThreads, 0);
