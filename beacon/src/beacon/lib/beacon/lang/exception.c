...
void bc_InitException() {
        ...
        bc_DefineNativeMethodClass(exceptionClass, "nativeInit",
                                   bc_exception_nativeInit);
}
...
static void bc_exception_nativeInit(bc_Method* parent, bc_Frame* fr,
        ...
        do {
                ...
                bc_LineRange* lr = bc_FindLineRange(
                    temp->ContextRef->LineRangeTable, temp->PC);
                int lineno = lr == NULL ? -1 : lr->Lineno;
                // assert(lineno != -1);
                //直前の表示と同じ
                if (lfilename != NULL &&
                    !strcmp(temp->ContextRef->ContextRef->FileName,
                            lfilename) &&
                    llineno == lineno) {
                        temp = temp->Parent;
                        continue;
                ...
                bc_PushVector(
                    args, bc_NewString(temp->ContextRef->ContextRef->FileName));
                bc_PushVector(args, bc_NewInteger(lineno));
                bc_Object* trace = bc_NewInstanceClass(stackTraceElementClass,
                                                       // ilctx,
                                                       fr, args, NULL);
        ...
        //配列へ
        bc_Object* arr =
            bc_DynamicNewArray(stackTraceElementClass->Parent->GenericSelf,
                               stackTraceElementVec->Length, fr);
