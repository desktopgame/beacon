...
static void bc_time_nativeGetRawTime(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	...
	time(&tref);
	//FIXME:long型を導入する
	bc_PushVector(fr->ValueStack, NewLong(tref));
}

static void bc_time_nativeToString(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* self = bc_AtVector(fr->VariableTable, 0);
	int temp;
	bc_FindFieldClass(BC_TYPE2CLASS(GetTimeType()), bc_InternString("rawTime"), &temp);
	bc_Object* rawTime = (bc_Object*)bc_AtVector(self->Fields, temp);
	bc_PushVector(fr->ValueStack, NewString(ctime(&(((Long*)rawTime)->Value))));
}

static void bc_time_nativeGetSecond(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_sec));
}

static void bc_time_nativeGetMinute(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_min));
}

static void bc_time_nativeGetHour(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_hour));
}

static void bc_time_nativeGetDay(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_mday));
}

static void bc_time_nativeGetYear(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_year));
}

static void bc_time_nativeGetDayOfWeek(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_wday));
}

static void bc_time_nativeGetDayOfYear(bc_Method* parent, bc_Frame* fr, bc_Enviroment* env) {
	bc_Object* rawTime = bc_AtVector(fr->VariableTable, 1);
	bc_PushVector(fr->ValueStack, bc_GetIntObject(localtime(&(((Long*)rawTime)->Value))->tm_yday));
}
