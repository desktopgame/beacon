...
void bc_GenerateILStmt(bc_ILStatement * self, bc_Enviroment* env, bc_CallContext* cctx) {
	...
	switch (self->Type) {
		...
		case ILSTMT_VARIABLE_DECL_T:
			bc_GenerateILVariableDecl(self->Kind.VariableDecl, env, cctx);
			break;
		case ILSTMT_VARIABLE_INIT_T:
			bc_GenerateILVariableInit(self->Kind.VariableInit, env, cctx);
			break;
		...
		case ILSTMT_INFERENCED_TYPE_INIT_T:
			bc_GenerateILInferencedTypeInit(self->Kind.InferencedTypeInit, env, cctx);
			break;
		case ILSTMT_TRY_T:
		...
		case ILSTMT_YIELD_RETURN_T:
			bc_GenerateILYieldReturn(self->Kind.YieldReturn, env, cctx);
			break;
		case ILSTMT_YIELD_BREAK_T:
			bc_GenerateILYieldBreak(self->Kind.YieldBreak, env, cctx);
			break;
...
void bc_LoadILStmt(bc_ILStatement * self, bc_Enviroment* env, bc_CallContext* cctx) {
	...
	switch (self->Type) {
		...
		case ILSTMT_PROC_T:
			...
			break;
		case ILSTMT_VARIABLE_DECL_T:
			bc_LoadILVariableDecl(self->Kind.VariableDecl, env, cctx);
			break;
		case ILSTMT_VARIABLE_INIT_T:
			bc_LoadILVariableInit(self->Kind.VariableInit, env, cctx);
			break;
		case ILSTMT_RETURN_T:
		...
		case ILSTMT_CONTINUE_T:
			...
			break;
		case ILSTMT_INFERENCED_TYPE_INIT_T:
			bc_LoadILInferencedTypeInit(self->Kind.InferencedTypeInit, env, cctx);
			break;
		case ILSTMT_TRY_T:
...
void bc_DeleteILStmt(bc_ILStatement * self) {
	switch (self->Type) {
		...
		case ILSTMT_CONTINUE_T:
			...
			break;
		case ILSTMT_INFERENCED_TYPE_INIT_T:
			bc_DeleteILInferencedTypeInit(self->Kind.InferencedTypeInit);
			break;
		case ILSTMT_TRY_T:
