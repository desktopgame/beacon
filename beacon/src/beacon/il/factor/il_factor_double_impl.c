...
void il_factor_double_generate(il_factor_double * self, enviroment* env, il_context* cache) {
	int index = enviroment_add_constant_double(env, self->value);
	opcode_buf_add(env->buf, op_dconst);
	opcode_buf_add(env->buf, index);
}

void il_factor_double_load(il_factor_double * self, enviroment * env, il_context* cache, il_ehandler * eh) {
}

generic_type* il_factor_double_eval(il_factor_double * self, enviroment * env, il_context* cache) {
