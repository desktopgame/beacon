...
// proto
static void ILMemberOp_check(bc_ILMemberOp* self, bc_Enviroment* env,
                             bc_CallContext* cctx, bool* swap);
static void ILMemberOp_check_static(bc_ILMemberOp* self, bc_Enviroment* env,
                                    bc_CallContext* cctx,
                                    bc_GenericType* receiver_type, bool* swap);
static void ILMemberOp_check_prop(bc_ILMemberOp* self, bc_Enviroment* env,
                                  bc_CallContext* cctx,
                                  bc_GenericType* receiver_type, bool* swap);
static void ILMemberOp_check_static_prop(bc_ILMemberOp* self,
                                         bc_Enviroment* env,
                                         bc_CallContext* cctx,
                                         bc_GenericType* receiver_type,
                                         bool* swap);
...
void bc_LoadILMemberOp(bc_ILMemberOp* self, bc_Enviroment* env,
                       bc_CallContext* cctx) {
        bool swap;
...
void bc_GenerateILMemberOp(bc_ILMemberOp* self, bc_Enviroment* env,
                           bc_CallContext* cctx) {
...
bc_GenericType* bc_EvalILMemberOp(bc_ILMemberOp* self, bc_Enviroment* env,
                                  bc_CallContext* cctx) {
        // ILMemberOp_checkは、
...
static void ILMemberOp_check_static(bc_ILMemberOp* self, bc_Enviroment* env,
        ...
        // Name.call
        //の Name を型名として解決する
        bc_GenericType* ref = bc_ResolvefImportManager(
            cctx->Scope, ilvar->Kind.Static->FQCN, cctx);
        receiver_type = ref;
        // Name.call
...
static void ILMemberOp_check_prop(bc_ILMemberOp* self, bc_Enviroment* env,
        ...
        //プロパティの可視性を確認
        if (temp == -1) {
                bc_Panic(BCERROR_UNDEFINED_PROPERTY_T,
                         bc_Ref2Str(bc_GetTypeName(ctype)),
