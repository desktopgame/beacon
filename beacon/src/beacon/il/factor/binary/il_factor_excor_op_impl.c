...
bc_GenericType* bc_EvalILExcorOp(bc_ILExcorOp * self, bc_Enviroment* env, bc_CallContext* cctx) {
...
void bc_GenerateILExcorOp(bc_ILExcorOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	//演算子オーバーロードが見つからない
	if(self->OperatorIndex == -1) {
...
void bc_LoadILExcorOp(bc_ILExcorOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	if(!bc_IsIntIntBinaryOp(self->Parent, env, cctx) &&
