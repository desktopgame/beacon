...
bc_ILLogicOp* bc_NewILLogicOp(bc_OperatorType type) {
        bc_ILLogicOp* ret = (bc_ILLogicOp*)MEM_MALLOC(sizeof(bc_ILLogicOp));
        ret->Type = type;
        ret->Parent = NULL;
        ret->OperatorIndex = -1;
        return ret;
...
bc_GenericType* bc_EvalILLogicOp(bc_ILLogicOp* self, bc_Enviroment* env,
        ...
        if (bc_IsIntIntBinaryOp(self->Parent, env, cctx)) {
                ...
                bc_GenericType* lgtype =
                    bc_EvalILFactor(self->Parent->Left, env, cctx);
                //プリミティブ型同士でないのに
                //演算子オーバーロードもない
                if (self->OperatorIndex == -1) {
                        bc_Panic(BCERROR_UNDEFINED_LOGIC_OPERATOR_T,
                ...
                return bc_ApplyILBinaryOp(self->Parent,
                                          operator_ov->ReturnGType, env, cctx);
        }
}

void bc_GenerateILLogicOp(bc_ILLogicOp* self, bc_Enviroment* env,
                          bc_CallContext* cctx) {
        if (self->OperatorIndex == -1) {
                ...
                if (bc_IsIntIntBinaryOp(self->Parent, env, cctx)) {
                        ...
                        bc_AddOpcodeBuf(
                            env->Bytecode,
                            (bc_VectorItem)operator_to_bopcode(self->Type));
                } else {
                        assert(false);
                }
        } else {
...
void bc_DeleteILLogicOp(bc_ILLogicOp* self) { MEM_FREE(self); }

char* bc_ILLogicOpToString(bc_ILLogicOp* self, bc_Enviroment* env) {
