...
bc_ILExplicitBinaryOp* bc_NewILExplicitBinaryOp(bc_OperatorType type) {
	bc_ILExplicitBinaryOp* ret = (bc_ILExplicitBinaryOp*)MEM_MALLOC(sizeof(bc_ILExplicitBinaryOp));
	ret->Type = type;
	ret->Receiver = NULL;
	ret->Arg = NULL;
	ret->Index = -1;
...
void bc_GenerateILExplicitBinaryOp(bc_ILExplicitBinaryOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
...
void bc_LoadILExplicitBinaryOp(bc_ILExplicitBinaryOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	bc_LoadILFactor(self->Receiver, env, cctx);
	bc_LoadILFactor(self->Arg, env, cctx);
	self->Index = bc_GetIndexILBinaryOp2(self->Receiver, self->Arg, self->Type, env, cctx);
	assert(self->Index != -1);
}

bc_GenericType* bc_EvalILExplicitBinaryOp(bc_ILExplicitBinaryOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
