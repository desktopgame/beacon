...
bc_ILExplicitUnaryOp* bc_NewILExplicitUnaryOp(bc_OperatorType type) {
        bc_ILExplicitUnaryOp* ret =
            (bc_ILExplicitUnaryOp*)MEM_MALLOC(sizeof(bc_ILExplicitUnaryOp));
        ret->Type = type;
        ret->Receiver = NULL;
        ret->Index = -1;
        return ret;
...
void bc_LoadILExplicitUnaryOp(bc_ILExplicitUnaryOp* self, bc_Enviroment* env,
                              bc_CallContext* cctx) {
        bc_LoadILFactor(self->Receiver, env, cctx);
        self->Index =
            bc_GetIndexILUnaryOp2(self->Receiver, self->Type, env, cctx);
        assert(self->Index != -1);
}

bc_GenericType* bc_EvalILExplicitUnaryOp(bc_ILExplicitUnaryOp* self,
                                         bc_Enviroment* env,
                                         bc_CallContext* cctx) {
