#include "il_factor_op_call_impl.h"
#include "../../util/mem.h"
#include "../../util/text.h"
...
//proto
static void il_factor_op_call_argument_delete(vector_item item);
static void il_factor_op_call_type_argument_delete(vector_item item);

il_factor* il_factor_wrap_op_call(il_factor_op_call* self) {
	il_factor* ret = (il_factor*)MEM_MALLOC(sizeof(il_factor));
	ret->type = ilfactor_op_call;
	ret->u.op_call_ = self;
	self->parent = ret;
	return ret;
}
...
il_factor_op_call* il_factor_op_call_new() {
	il_factor_op_call* ret = (il_factor_op_call*)MEM_MALLOC(sizeof(il_factor_op_call));
	ret->receiver = NULL;
...
generic_type* il_factor_op_call_eval(il_factor_op_call* self, struct enviroment* env, il_load_cache* cache) {
	il_factor_op_call_load(self, env, cache, NULL);
