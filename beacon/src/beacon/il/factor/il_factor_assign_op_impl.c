...
static void assign_by_namebase(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
static void assign_to_field(ILAssignOp* self,bc_ILFactor* receiver, bc_ILFactor* source, bc_StringView namev, bc_Enviroment* env, bc_CallContext* cctx);
static void assign_to_Property(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
static void assign_to_array(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
static void assign_by_call(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
...
static bool can_assign_to_field(bc_Field* f, ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
static void generate_assign_to_variable(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
static void generate_assign_to_variable_local(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx);
...
bc_GenericType* EvalILAssignOp(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
...
static void assign_by_namebase(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	...
	if(ilvar->Type == ILVARIABLE_TYPE_STATIC_T) {
		bc_Class* cls = BC_TYPE2CLASS(bc_GetEvalTypeCContext(cctx, ilvar->Kind.Static->FQCN));
		int temp = -1;
...
static void assign_to_Property(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	...
	if(!bc_IsAccessiblePropertyClass(bc_GetClassCContext(cctx), pp)) {
		bc_Panic(BCERROR_CAN_T_ACCESS_FIELD_T,
			bc_Ref2Str(bc_GetTypeName(pp->Parent)),
			bc_Ref2Str(pp->Name)
		);
		return;
	}
	if(!bc_IsAccessiblePropertyAccessorClass(bc_GetClassCContext(cctx), pp->Set)) {
		bc_Panic(BCERROR_CAN_T_ACCESS_FIELD_T,
			bc_Ref2Str(bc_GetTypeName(pp->Parent)),
			bc_Ref2Str(pp->Name)
		);
	...
	if(bc_DistanceGenericType(prop->Property->GType, bc_EvalILFactor(self->Right, env, cctx), cctx) < 0) {
		bc_Panic(BCERROR_ASSIGN_NOT_COMPATIBLE_PROPERTY_T,
			bc_Ref2Str(bc_GetTypeName(prop->Property->Parent)),
...
static void generate_assign_to_variable_local(ILAssignOp* self, bc_Enviroment* env, bc_CallContext* cctx) {
	...
	if(illoc->Type == VARIABLE_LOCAL_SCOPE_T) {
		...
		#endif
		bc_SymbolEntry* e = bc_EntrySymbolTable(env->Symboles, NULL, ilvar->FQCN->Name);
		//e==NULL の時変数がない
		bc_GenerateILFactor(self->Right, env, cctx);
		bc_AddOpcodeBuf(env->Bytecode, OP_STORE);
		bc_AddOpcodeBuf(env->Bytecode, e->Index);
		if(bc_DistanceGenericType(e->GType, bc_EvalILFactor(self->Right, env, cctx), cctx) < 0) {
	...
	//src のような名前がフィールドを示す場合
		...
		int temp = -1;
		bc_Field* f = bc_FindTreeFieldClass(bc_GetClassCContext(cctx), illoc->Name, &temp);
		if(temp == -1) {
			f = bc_FindTreeSFieldClass(bc_GetClassCContext(cctx), illoc->Name, &temp);
		}
		assert(temp != -1);
		//フィールドはstaticでないが
	...
	//src のような名前がプロパティを示す場合
		...
		int temp = -1;
		bc_Property* p = bc_FindTreePropertyClass(bc_GetClassCContext(cctx), illoc->Name, &temp);
		assert(temp != -1);
		//フィールドはstaticでないが
		//現在のコンテキストはstaticなので this にアクセスできない
		if(!bc_IsStaticModifier(p->Modifier) &&
		    bc_IsStaticCContext(cctx)) {
			bc_Panic(BCERROR_ACCESS_TO_THIS_AT_STATIC_METHOD_T,
				bc_Ref2Str(bc_GetTypeName(p->Parent)),
				bc_Ref2Str(p->Name)
			);
			return;
		}
