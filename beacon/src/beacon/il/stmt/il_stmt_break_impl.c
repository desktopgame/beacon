...
void il_stmt_break_generate(void * empty, enviroment * env, il_context* cache) {
	label* lab = (label*)vector_top(cache->while_end_vec);
	opcode_buf_add(env->buf, op_goto);
	opcode_buf_add(env->buf, lab);
}

void il_stmt_break_load(void * empty, enviroment * env, il_context* cache, il_ehandler* eh) {
}
