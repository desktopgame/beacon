...
void il_stmt_continue_generate(void * empty, enviroment * env, il_context* cache) {
	label* lab = (label*)vector_top(cache->while_start_vec);
	opcode_buf_add(env->buf, op_goto);
	opcode_buf_add(env->buf, lab);
}

void il_stmt_continue_load(void * empty, enviroment * env, il_context* cache, il_ehandler* eh) {
}
