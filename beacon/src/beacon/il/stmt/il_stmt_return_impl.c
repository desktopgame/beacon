...
static void check_method_return(bc_ILReturn* self, bc_Enviroment* env,
                                bc_CallContext* cctx);
...
void bc_GenerateILReturn(bc_ILReturn* self, bc_Enviroment* env,
                         bc_CallContext* cctx) {
        bc_GenerateILFactor(self->Factor, env, cctx);
        bc_AddOpcodeBuf(env->Bytecode, OP_RETURN);
}

void bc_LoadILReturn(bc_ILReturn* self, bc_Enviroment* env,
                     bc_CallContext* cctx) {
...
static void check_method_return(bc_ILReturn* self, bc_Enviroment* env,
        ...
        if (retT->CoreType != BC_TYPE_NULL &&
            bc_CdistanceGenericType(m->ReturnGType, retT) < 0) {
                bc_Panic(
