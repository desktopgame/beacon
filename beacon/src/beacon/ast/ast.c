...
bc_AST* bc_NewASTScopeEmpty() { return bc_NewASTScope(bc_NewASTBlank()); }

bc_AST* bc_NewASTBlank() { return bc_NewAST(AST_BLANK_T); }
...
bc_AST* bc_FirstAST(bc_AST* self) { return bc_AtAST(self, 0); }

bc_AST* bc_SecondAST(bc_AST* self) { return bc_AtAST(self, 1); }
...
bool bc_IsAccessAST(bc_AST* self) { return self->Tag == AST_ACCESS_LEVEL_T; }

bool bc_IsModifierAST(bc_AST* self) { return self->Tag == AST_MOD_Tifier; }
...
static bc_ModifierType ast_to_modifier(bc_AST* self, bool* error) {
        ...
        for (int i = 0; i < self->Children->Length; i++) {
                ...
                if (ret == -1) {
                        ...
                        //追加の属性がすでに含まれているかどうか
                        bc_ModifierType add =
                            bc_ASTCastToModifier(bc_AtAST(self, i), error);
                        if ((ret & add) > 0) {
                                (*error) = true;
                        }
                        ret |= add;
