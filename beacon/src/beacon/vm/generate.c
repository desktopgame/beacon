...
void bc_GenerateGetProperty(bc_OpcodeBuf* buf, bc_Property* prop, int index) {
	if(prop->IsShort) {
		bc_GenerateGetField(buf, prop->SourceRef, bc_GetFieldByPropertyClass(prop->Parent->Kind.Class, prop));
	} else {
...
void bc_GeneratePutProperty(bc_OpcodeBuf* buf, bc_Property* prop, int index) {
	if(prop->IsShort) {
		bc_GeneratePutField(buf, prop->SourceRef, bc_GetFieldByPropertyClass(prop->Parent->Kind.Class, prop));
	} else {
		if(bc_IsStaticModifier(prop->Modifier)) {
