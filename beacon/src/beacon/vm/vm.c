...
void bc_ResumeVM(bc_Frame* self, bc_Enviroment * env, int pos) {
	...
	while(self->DeferList->Length > 0) {
		bc_DeferContext* defctx = (bc_DeferContext*)bc_PopVector(self->DeferList);
...
static void vm_run(bc_Frame* self, bc_Enviroment * env, int pos, int deferStart) {
	...
	for (int IDX = pos; IDX < source_len; IDX++) {
		...
		switch (b) {
			//int & int
			case OP_IADD:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetIntObject(SPI(self) + SPI(self))));
				break;
			case OP_ISUB:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetIntObject(SPI(self) - SPI(self))));
				break;
			...
			case OP_IDIV:
			{
				...
				int b = SPI(self);
				assert(b != 0);
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetIntObject(a / b)));
				break;
			}
			case OP_IMOD:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetIntObject(SPI(self) % SPI(self))));
			...
			case OP_IEQ:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(SPI(self) == SPI(self))));
				break;
			case OP_INOTEQ:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(SPI(self) != SPI(self))));
				break;
			...
			case OP_ILT:
			{
				...
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(bc_ObjectToInt(a_) < bc_ObjectToInt(b_))));
			...
			case OP_IFLIP:
				...
				break;
			case OP_CEQ:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(SPC(self) == SPC(self))));
				break;
			case OP_CNOTEQ:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(SPC(self) != SPC(self))));
				break;
			...
			{
				bool a = SPB(self);
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(!a)));
				break;
			}
				//TODO:短絡評価していない
			case OP_BBIT_OR:
			{
				bool ab = SPB(self);
				bool bb = SPB(self);
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(ab | bb)));
				break;
			}
			case OP_BLOGIC_OR:
			{
			...
			case OP_BEXCOR:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(SPB(self) ^ SPB(self))));
				break;
			case OP_BFLIP:
				bc_PushVector(self->ValueStack, NON_NULL(bc_GetBoolObject(~SPB(self))));
				break;
				//push const
			case OP_ICONST:
			{
			...
			case OP_TRUE:
			{
				bc_PushVector(self->ValueStack, bc_GetTrueObject());
				break;
			}
			case OP_FALSE:
			{
				bc_PushVector(self->ValueStack, bc_GetFalseObject());
				break;
			}
			case OP_NULL:
			{
			...
			case OP_HEXCEPTION:
			{
				bc_PushVector(self->ValueStack, NON_NULL(self->Exception));
				break;
			}
			case OP_INSTANCEOF:
			{
				bc_GenericType* gtype = (bc_GenericType*)bc_PopVector(self->TypeArgs);
				bc_Object* v = (bc_Object*)bc_PopVector(self->ValueStack);
			...
			case OP_DUP:
				bc_PushVector(self->ValueStack, bc_TopVector(self->ValueStack));
				break;
			case OP_POP:
			...
			case OP_ALLOC_FIELD:
			{
				...
				bc_Type* tp = (bc_Type*)bc_AtVector(ctx->TypeList, absClsIndex);
				bc_Class* cls = BC_TYPE2CLASS(tp);
				bc_Object* obj = (bc_Object*)bc_TopVector(self->ValueStack);
				//仮想関数テーブル更新
				...
				if(cls->TypeParameters->Length == 0) {
					...
					for(int i=0; i<cls->TypeParameters->Length; i++) {
						bc_AddArgsGenericType(g, (bc_GenericType*)bc_AtVector(self->TypeArgs, i));
					}
					obj->GType = g;
				}
			...
			case OP_NEW_INSTANCE:
			{
				...
				#endif
				bc_Constructor* ctor = (bc_Constructor*)bc_AtVector(cls->Constructors, constructorIndex);
				//新しいVMでコンストラクタを実行
				//また、現在のVMから実引数をポップ
				...
				for (int i = 0; i < ctor->Parameters->Length; i++) {
					bc_VectorItem e = bc_PopVector(self->ValueStack);
					bc_Object* o = (bc_Object*)e;
					bc_PushVector(sub->ValueStack, NON_NULL(e));
					bc_AssignVector(cfr->Kind.StaticInvoke.Args, (ctor->Parameters->Length - i), o->GType);
				}
				//コンストラクタに渡された型引数を引き継ぐ
				int typeparams = cls->TypeParameters->Length;
				for(int i=0; i<typeparams; i++) {
					bc_VectorItem e = bc_PopVector(self->TypeArgs);
					bc_AssignVector(sub->TypeArgs, (typeparams - i) - 1, e);
					bc_AssignVector(cfr->Kind.StaticInvoke.TypeArgs, (cls->TypeParameters->Length - i), e);
				}
				...
				bc_PushVector(self->ValueStack, NON_NULL(returnV));
				bc_DeleteFrame(sub);
				break;
			}
			case OP_CHAIN_THIS:
			case OP_CHAIN_SUPER:
			{
				...
				bc_Constructor* ctor = (bc_Constructor*)bc_AtVector(cls->Constructors, ctorIndex);
				...
				for (int i = 0; i < ctor->Parameters->Length; i++) {
					bc_Object* o = (bc_Object*)bc_PopVector(self->ValueStack);
					bc_PushVector(sub->ValueStack, NON_NULL(o));
					bc_AssignVector(cfr->Kind.StaticInvoke.Args, (ctor->Parameters->Length - i), o->GType);
				}
				for(int i=0; i<self->TypeArgs->Length; i++) {
					bc_PushVector(cfr->Kind.StaticInvoke.TypeArgs, self->TypeArgs);
				}
				//		DumpEnviromentOp(ctor->env, sub->level);
				//DumpOpcodeBuf(ctor->env->Bytecode, sub->level);
				bc_ExecuteVM(sub, ctor->Env);
				...
				bc_VectorItem returnV = bc_TopVector(sub->ValueStack);
				bc_Object* returnO = (bc_Object*)returnV;
				bc_AssignVector(self->VariableTable, 0, returnV);
				bc_PushVector(self->ValueStack, NON_NULL(returnV));
			...
			case OP_THIS:
			{
				bc_PushVector(self->ValueStack, bc_AtVector(self->VariableTable, 0));
				break;
			}
			case OP_SUPER:
			{
				bc_Object* a = bc_AtVector(self->VariableTable, 0);
			...
			case OP_PUT_FIELD:
			{
				bc_Object* assignValue = (bc_Object*)bc_PopVector(self->ValueStack);
				bc_Object* assignTarget = (bc_Object*)bc_PopVector(self->ValueStack);
				if(throw_npe(self, assignTarget)) {
					break;
				}
				//assert(assignTarget->Tag == OBJECT_REF_T);
				int fieldIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				bc_AssignVector(assignTarget->Fields, fieldIndex, assignValue);
				break;
			}

			case OP_GET_FIELD:
			{
				bc_Object* sourceObject = (bc_Object*)bc_PopVector(self->ValueStack);
				...
				int fieldIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				bc_Object* val = (bc_Object*)bc_AtVector(sourceObject->Fields, fieldIndex);
			...
			case OP_PUT_STATIC:
			{
				...
				bc_Object* sv = (bc_Object*)bc_PopVector(self->ValueStack);
				f->StaticValue = sv;
				break;
			}
			...
			case OP_GET_STATIC:
			{
				int absClsIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				int fieldIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				bc_Type* cls = (bc_Type*)bc_AtVector(ctx->TypeList, absClsIndex);
				bc_Field* f = bc_GetSFieldClass(cls->Kind.Class, fieldIndex);
				bc_PushVector(self->ValueStack, NON_NULL(f->StaticValue));
				break;
			}
			...
			{
				int index = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				bc_VectorItem e = bc_AtVector(self->VariableTable, index);
				bc_Object*  o = (bc_Object*)e;
				assert(o != NULL);
			...
			case OP_DOWN_AS:
			{
				...
				if(a->CoreType->Tag == TYPE_INTERFACE_T) {
					...
					int iter = bc_FindVector(inter_list, inter);
			...
			case OP_INVOKEINTERFACE:
			{
				...
				bc_Type* tp = bc_AtVector(ctx->TypeList, absClassIndex);
				bc_Object* o = (bc_Object*)bc_TopVector(self->ValueStack);
				if(throw_npe(self, o)) {
					break;
				}
			...
			case OP_INVOKESTATIC:
			{
				int absClassIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				int methodIndex = (int)bc_GetEnviromentSourceAt(env, ++IDX);
				bc_Type* cls = (bc_Type*)bc_AtVector(ctx->TypeList, absClassIndex);
				bc_Method* m = bc_GetSMethodClass(cls->Kind.Class, methodIndex);
				#if defined(DEBUG)
				const char* clsname = bc_Ref2Str(bc_GetTypeName(cls));
			...
			case OP_INVOKESPECIAL:
			{
				...
				bc_Method* m = (bc_Method*)bc_AtVector(cl->Methods, index);
				bc_ExecuteMethod(m, self, env);
				break;
			}
			case OP_INVOKEOPERATOR:
			{
				...
				bc_OperatorOverload* operator_ov = (bc_OperatorOverload*)bc_AtVector(cl->OVT->Operators, index);
				bc_ExecuteOperatorOverload(operator_ov, self, env);
				break;
			}
			case OP_CORO_INIT:
			{
				bc_Object* obj = (bc_Object*)bc_AtVector(self->VariableTable, 0);
				...
				yctx->Source = bc_AtVector(self->VariableTable, 1);
				#if defined(DEBUG)
				const char* yname = bc_GetObjectName(yctx->Source);
				#endif
				for(int i=2; i<param_len + 1; i++) {
					bc_Object* a = bc_AtVector(self->VariableTable, i);
					assert(a != NULL);
			...
			case OP_CORO_NEXT:
			{
				...
				if(yctx->VariableTable == NULL) {
					yctx->BackupVariableTable = bc_CloneVector(self->VariableTable);
			...
			case OP_CORO_SWAP_SELF:
			{
				...
				//[0] = Array
				//[1] = ArrayIterator
				bc_AssignVector(self->VariableTable, 0, yctx->Source);
				assert(yctx->Source != NULL);
				for(int i=0; i<yctx->Parameters->Length; i++) {
					bc_Object* e = bc_AtVector(yctx->Parameters, i);
					assert(e != NULL);
					bc_AssignVector(self->VariableTable, i + 1, e);
				}
				self->Coroutine = obj;
				yctx->IsCached = true;
				break;
			...
			case OP_GENERIC_ADD:
			{
				...
				while(1) {
					...
					if(code == OP_GENERIC_ENTER) {
						...
						depth--;
						int count = (int)bc_PopVector(counts);
						bc_GenericType* head = bc_AtVector(stack, 0);
						for(int i=0; i<count; i++) {
							bc_AddArgsGenericType(head, (bc_GenericType*)bc_PopVector(stack));
						}
						if(depth == 0) {
						...
						if(code == OP_GENERIC_UNIQUE_TYPE) {
							a = bc_NewGenericType((bc_Type*)bc_AtVector(ctx->TypeList, arg));
						} else if(code == OP_GENERIC_INSTANCE_TYPE) {
							bc_Object* receiver = (bc_Object*)bc_AtVector(self->VariableTable, 0);
							a = (bc_GenericType*)bc_AtVector(receiver->GType->TypeArgs, arg);
						} else if(code == OP_GENERIC_STATIC_TYPE) {
							a = (bc_GenericType*)bc_AtVector(self->TypeArgs, arg);
						}
			...
			case OP_DEFER_REGISTER:
			{
				...
				bc_Vector* bind = bc_CloneVector(self->VariableTable);
...
static char* create_error_message(bc_Frame* self, bc_Enviroment* env, int pc) {
	...
	//例外のメッセージを取得
	bc_Type* exceptionT = bc_FindTypeFromNamespace(bc_GetLangNamespace(), bc_InternString("Exception"));
	int temp = -1;
	...
	bc_AppendBuffer(sbuf, '\n');
	//スタックトレースの表示
	bc_Type* stackTraceElementT = bc_FindTypeFromNamespace(bc_GetLangNamespace(), bc_InternString("StackTraceElement"));
	//Exception#stackTraceを取得
	temp = -1;
	bc_FindFieldClass(exceptionT->Kind.Class, bc_InternString("stackTrace"), &temp);
