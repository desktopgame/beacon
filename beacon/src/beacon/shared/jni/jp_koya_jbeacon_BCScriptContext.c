...
Java_jp_koya_jbeacon_BCScriptContext_nativeOpen(JNIEnv* env, jclass cls) {
        ...
        jclass scontext_cls =
            (*env)->FindClass(env, "jp/koya/jbeacon/BCScriptContext");
        if (scontext_cls == NULL) {
                (*env)->FatalError(
                    env, "not found class: jp/koya/jbeacon/BCScriptContext");
                return NULL;
        }
        //コンストラクタを検索
        jmethodID scontext_ctor_id =
            (*env)->GetMethodID(env, scontext_cls, "<init>", "([B)V");
        ...
        (*env)->SetByteArrayRegion(env, pointer, 0, sizeof(void*),
                                   (jbyte*)&sctx);
