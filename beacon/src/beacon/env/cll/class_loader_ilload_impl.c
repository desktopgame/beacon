 ...
 * @param source
 */
static void class_loader_ilload_import_list(bc_ClassLoader* self,
                                            bc_AST* asource);
 ...
 * @param import_decl
 */
static void class_loader_ilload_import(bc_ClassLoader* self,
                                       bc_AST* aimport_decl);
 ...
 * @param Namespacedecl
 */
static void class_loader_ilload_namespace(bc_ClassLoader* self,
                                          bc_Vector* parent,
 ...
 * @param Namespacebody
 */
static void class_loader_ilload_Namespacepath_recursive(bc_ClassLoader* self,
                                                        bc_AST* aNamespacepath,
                                                        bc_AST* Namespacebody);
 ...
 * @return
 */
static bc_ILNamespace* class_loader_ilload_ast_to_namespace(bc_AST* a);
 ...
 * @param Namespacebody
 */
static void class_loader_ilload_Namespacebody(bc_ClassLoader* self,
 ...
 * @param class_decl
 */
static void class_loader_ilload_abstract_class(bc_ClassLoader* self,
                                               bc_ILNamespace* current,
                                               bc_AST* aclass_decl);
static void class_loader_ilload_class(bc_ClassLoader* self,
                                      bc_ILNamespace* current,
                                      bc_AST* aclass_decl);
static bc_ILClass* class_loader_ilload_classImpl(bc_ClassLoader* self,
                                                 bc_ILNamespace* current,
                                                 bc_AST* aclass_decl);
static void class_loader_ilload_interface(bc_ClassLoader* self,
                                          bc_ILNamespace* current,
                                          bc_AST* ainterface_decl);
static void class_loader_ilload_enum(bc_ClassLoader* self,
                                     bc_ILNamespace* current,
                                     bc_AST* aenum_decl);
 ...
 * @param source
 */
static void class_loader_ilload_identifier_list(bc_ClassLoader* self,
                                                bc_Vector* list,
                                                bc_AST* asource);

// static ILFactor* CLILFactorImpl(ClassLoader* self, AST* source);
// static ILStatement* class_loader_ilload_bodyImpl(ClassLoader* self, AST*
// source);

void ILLoadClassLoader(bc_ClassLoader* self, bc_AST* source_code) {
        assert(self->ILCode == NULL);
        self->ILCode = bc_NewILToplevel();
...
// private
static void class_loader_ilload_function(bc_ClassLoader* self,
                                         bc_AST* asource) {
...
static void class_loader_ilload_import_list(bc_ClassLoader* self,
        ...
        if (asource->Tag == AST_IMPORT_DECL_LIST_T) {
                for (int i = 0; i < asource->Children->Length; i++) {
                        class_loader_ilload_import_list(self,
                                                        bc_AtAST(asource, i));
                }
        } else {
...
static void class_loader_ilload_namespace(bc_ClassLoader* self,
        ...
        bc_ILNamespace* iln =
            class_loader_ilload_ast_to_namespace(aNamespacepath);
        bc_ILNamespace* top = bc_GetRootILNamespace(iln);
        bc_PushVector(parent, top);
        class_loader_ilload_Namespacepath_recursive(self, aNamespacepath,
                                                    aNamespacebody);
        class_loader_ilload_Namespacebody(self, iln, iln->NamespaceList,
                                          aNamespacebody);
}

static void class_loader_ilload_Namespacepath_recursive(
    bc_ClassLoader* self, bc_AST* aNamespacepath, bc_AST* aNamespacebody) {
        ...
        if (aNamespacepath->Tag == AST_NAMESPACE_PATH_T) {
                ...
                for (int i = 0; i < aNamespacepath->Children->Length; i++) {
                        class_loader_ilload_Namespacepath_recursive(
                            self, bc_AtAST(aNamespacepath, i), aNamespacebody);
                }
        }
}
...
static bc_ILNamespace* class_loader_ilload_ast_to_namespace(bc_AST* a) {
        ...
        if (a->Tag == AST_NAMESPACE_PATH_T) {
                ...
                bc_ILNamespace* parent =
                    class_loader_ilload_ast_to_namespace(al);
                bc_ILNamespace* child =
                    class_loader_ilload_ast_to_namespace(ar);
                child->Parent = parent;
...
static void class_loader_ilload_Namespacebody(bc_ClassLoader* self,
        ...
        if (aNamespacebody->Tag == AST_NAMESPACE_DECL_T) {
                ...
                class_loader_ilload_abstract_class(self, current,
                                                   aNamespacebody);
                ...
                for (int i = 0; i < aNamespacebody->Children->Length; i++) {
                        ...
                        class_loader_ilload_Namespacebody(self, current, parent,
                                                          amember);
                }
        }
}
...
static void class_loader_ilload_abstract_class(bc_ClassLoader* self,
        ...
        bc_ILClass* ilc =
            class_loader_ilload_classImpl(self, current, aclass_decl);
        ilc->IsAbstract = true;
}
...
static bc_ILClass* class_loader_ilload_classImpl(bc_ClassLoader* self,
        ...
        // class Foo<A, B>
        CLILTypeParameter(self, bc_FirstAST(atypename),
...
static void class_loader_ilload_identifier_list(bc_ClassLoader* self,
        ...
        if (asource->Tag == AST_IDENTIFIER_LIST_T) {
                for (int i = 0; i < asource->Children->Length; i++) {
                        class_loader_ilload_identifier_list(
                            self, list, bc_AtAST(asource, i));
                }
