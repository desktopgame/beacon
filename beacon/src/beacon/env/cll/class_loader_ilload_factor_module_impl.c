...
static bc_ILUnaryOp* CLIL_unary(bc_ClassLoader* self, bc_AST* source, bc_OperatorType type);
static bc_ILBinaryOp* CLIL_binary(bc_ClassLoader* self, bc_AST* source, bc_OperatorType type);
static bc_ILExplicitUnaryOp* CLIL_explicit_unary(bc_ClassLoader* self, bc_AST* source, bc_OperatorType type);
static bc_ILExplicitBinaryOp* CLIL_explicit_binary(bc_ClassLoader* self, bc_AST* source, bc_OperatorType type);
static bc_ILAssignOp* CLIL_assign(bc_ClassLoader* self, bc_AST* source);
static bc_ILAssignOp* CLIL_assign_arithmetic(bc_ClassLoader* self, bc_AST* source, bc_OperatorType type);
static bc_ILVariable* CLIL_variable(bc_ClassLoader* self, bc_AST* source);
static bc_ILNewInstance* CLIL_new_instance(bc_ClassLoader* self, bc_AST* source);
...
static bc_ILFactor* CLILFactorImpl(bc_ClassLoader* self, bc_AST* source) {
	if (source->Tag == AST_INT_T) {
		...
		return bc_WrapILDouble(bc_NewILDouble(source->Attr.DoubleValue));
	} else if (source->Tag == AST_CHAR_T) {
		return bc_WrapILChar(bc_NewILChar(source->Attr.CharValue));
	} else if (source->Tag == AST_STRING_T) {
		return bc_WrapILString(bc_NewILString(source->Attr.StringVValue));
	} else if (source->Tag == AST_VARIABLE_T) {
		return bc_WrapILVariable(CLIL_variable(self, source));
		//operator(+ - * / %)
	} else if (source->Tag == AST_ADD_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_ADD_T));
	} else if (source->Tag == AST_SUB_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_SUB_T));
	} else if (source->Tag == AST_MUL_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_MUL_T));
	} else if (source->Tag == AST_DIV_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_DIV_T));
	} else if (source->Tag == AST_MOD_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_MOD_T));
		//operator(| || & &&)
	} else if (source->Tag == AST_BIT_OR_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_BIT_OR_T));
	} else if (source->Tag == AST_LOGIC_OR_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_LOGIC_OR_T));
	} else if (source->Tag == AST_BIT_AND_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_BIT_AND_T));
	} else if (source->Tag == AST_LOGIC_AND_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_LOGIC_AND_T));
	//^
	} else if(source->Tag == AST_EXC_OR_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_EXCOR_T));
	} else if(source->Tag == AST_CHILDA_T) {
		return bc_WrapILUnaryOp(CLIL_unary(self, source, OPERATOR_CHILDA_T));
	//<< >>
	} else if(source->Tag == AST_LSHIFT_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_LSHIFT_T));
	} else if(source->Tag == AST_RSHIFT_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_RSHIFT_T));
	//operator(== != > >= < <=)
	} else if (source->Tag == AST_EQUAL_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_EQ_T));
	} else if (source->Tag == AST_NOT_TEQUAL_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_NOT_EQ_T));
	} else if (source->Tag == AST_GT_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_GT_T));
	} else if (source->Tag == AST_GE_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_GE_T));
	} else if (source->Tag == AST_LT_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_LT_T));
	} else if (source->Tag == AST_LE_T) {
		return bc_WrapILBinaryOp(CLIL_binary(self, source, OPERATOR_LE_T));
		//operator(= += -= *= /= %=)
	} else if (source->Tag == AST_AS_TSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign(self, source));
	} else if (source->Tag == AST_ADD_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_ADD_T));
	} else if (source->Tag == AST_SUB_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_SUB_T));
	} else if (source->Tag == AST_MUL_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_MUL_T));
	} else if (source->Tag == AST_DIV_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_DIV_T));
	} else if (source->Tag == AST_MOD_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_MOD_T));
	//instanceof
	...
	//|= &=
		...
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_BIT_OR_T));
	} else if(source->Tag == AST_AND_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_BIT_AND_T));
	//<<= >>=
	} else if(source->Tag == AST_LSHIFT_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_LSHIFT_T));
	} else if(source->Tag == AST_RSHIFT_ASSIGN_T) {
		return bc_WrapILAssignOp(CLIL_assign_arithmetic(self, source, OPERATOR_RSHIFT_T));
	//!-
	} else if (source->Tag == AST_NOT_T) {
		return bc_WrapILUnaryOp(CLIL_unary(self, source, OPERATOR_NOT_T));
	} else if (source->Tag == AST_NEG_T) {
		return bc_WrapILUnaryOp(CLIL_unary(self, source, OPERATOR_NEGATIVE_T));
	} else if(source->Tag == AST_EXPLICIT_UOPERATOR_T) {
		return bc_WrapILExplicitUnaryOp(CLIL_explicit_unary(self, source, source->Attr.OperatorValue));
	} else if(source->Tag == AST_EXPLICIT_BIOPERATOR_T) {
		return bc_WrapILExplicitBinaryOp(CLIL_explicit_binary(self, source, source->Attr.OperatorValue));
		//this super
