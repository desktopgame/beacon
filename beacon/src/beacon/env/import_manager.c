...
bc_ImportManager* bc_NewImportManager() {
        bc_ImportManager* ret =
            (bc_ImportManager*)MEM_MALLOC(sizeof(bc_ImportManager));
        ret->Items = bc_NewVector();
        return ret;
}

bc_ImportInfo* bc_ImportImportManager(bc_ImportManager* self,
                                      bc_ClassLoader* target) {
...
bc_GenericType* bc_ResolveImportManager(bc_Namespace* scope,
                                        bc_GenericCache* fqcn,
                                        bc_CallContext* cctx) {
        ...
        if (CoreType != NULL && fqcn->TypeArgs->Length > 0) {
                ...
                for (int i = 0; i < fqcn->TypeArgs->Length; i++) {
                        bc_GenericCache* e =
                            (bc_GenericCache*)bc_AtVector(fqcn->TypeArgs, i);
                        bc_GenericType* child =
                            bc_ResolveImportManager(scope, e, cctx);
        ...
        if (parameterized->VirtualTypeIndex == -1 && ty != NULL) {
                ...
                parameterized->VirtualTypeIndex =
                    bc_GetGenericIndexType(ty, fqcn->FQCN->Name);
...
bc_GenericType* bc_ResolvefImportManager(bc_Namespace* scope,
                                         bc_FQCNCache* fqcn,
                                         bc_CallContext* cctx) {
