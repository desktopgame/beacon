...
bool bc_IsImplementInterfacePropertyValidClass(bc_Class* cls,
                                               bc_Property** out) {
        (*out) = NULL;
        //全ての実装インターフェイスを取得する
...
bool bc_IsMethodParameterValidClass(bc_Class* cls, bc_Method** out_method,
                                    bc_StringView* out_name) {
...
bool bc_IsConstructorParameterValidClass(bc_Class* self,
                                         bc_Constructor** out_ctor,
                                         bc_StringView* out_name) {
...
bool bc_IsTypeParameterValidClass(bc_Class* self, bc_StringView* out_name) {
        return !bc_IsOverwrappedTypeParameterName(self->TypeParameters,
                                                  out_name);
}

bool bc_IsMethodTypeParameterValidClass(bc_Class* self, bc_Method** out_method,
                                        bc_StringView* out_name) {
        (*out_name) = BC_ZERO_VIEW;
        for (int i = 0; i < self->Methods->Length; i++) {
                bc_Method* m = (bc_Method*)bc_AtVector(self->Methods, i);
                if (bc_IsOverwrappedTypeParameterName(m->TypeParameters,
                                                      out_name)) {
                        (*out_method) = m;
                        return false;
                }
...
static bool IsValidPropertyClassImpl(bc_Vector* prop_vec, bc_Property** out) {
        ...
        for (int i = 0; i < prop_vec->Length; i++) {
                ...
                for (int j = 0; j < prop_vec->Length; j++) {
                        bc_Property* pE =
                            (bc_Property*)bc_AtVector(prop_vec, j);
                        if (i == j) {
                                continue;
                        }
                        if (p->Name == pE->Name) {
...
#if defined(DEBUG)
                const char* tyname =
                    bc_Ref2Str(bc_GetTypeName(BC_MEMBER_TYPE(e)));
#endif
