...
static void delete_native_method_ref(bc_NumericMapKey key,
                                     bc_NumericMapItem item);
static bc_Method* class_find_impl_method(bc_Class* self,
                                         bc_Method* virtualMethod);
        ...
        for (int i = 0; i < self->Fields->Length; i++) {
                ...
                if (f->InitialValue != NULL) {
                        ...
                        for (int i = 0; i < fr->TypeArgs->Length; i++) {
                                bc_PushVector(sub->TypeArgs,
                                              bc_AtVector(fr->TypeArgs, i));
                        }
...
//それはフィールドと同じなのでフィールドも定義する
        ...
        if (p->IsShort) {
                bc_Field* f =
                    bc_NewField(bc_ConcatIntern("$propery.", p->Name));
                BC_MEMBER_ACCESS(f) = ACCESS_PRIVATE_T;
                f->GType = p->GType;
                BC_MEMBER_MODIFIER(f) = BC_MEMBER_MODIFIER(p);
...
void bc_DefineNativeMethodClass(bc_Class* self, const char* name,
                                bc_NativeImpl impl) {
        bc_DefineNativeMethodByRefClass(self, bc_InternString(name), impl);
}

void bc_DefineNativeMethodByRefClass(bc_Class* self, bc_StringView namev,
                                     bc_NativeImpl impl) {
...
void bc_CreateOperatorVTClass(bc_Class* self) {
        ...
        if (self->SuperClass != NULL) {
                bc_CreateOperatorVTClass(
                    BC_TYPE2CLASS(bc_GENERIC2TYPE(self->SuperClass)));
        }
        self->OVT = bc_NewOperatorVt();
        if (self->SuperClass == NULL) {
                for (int i = 0; i < self->OperatorOverloads->Length; i++) {
                        bc_OperatorOverload* opov =
                            bc_AtVector(self->OperatorOverloads, i);
                        bc_PushVector(self->OVT->Operators, opov);
                }
        } else {
                bc_OperatorVT* super_vt =
                    BC_TYPE2CLASS(bc_GENERIC2TYPE(self->SuperClass))->OVT;
                for (int i = 0; i < super_vt->Operators->Length; i++) {
                        bc_OperatorOverload* opov =
                            bc_AtVector(super_vt->Operators, i);
                        bc_PushVector(self->OVT->Operators, opov);
                }
                for (int i = 0; i < self->OperatorOverloads->Length; i++) {
                        bc_OperatorOverload* opov =
...
void bc_LinkAllClass(bc_Class* self) {
        ...
        for (int i = 0; i < self->Constructors->Length; i++) {
                bc_Constructor* ctor =
                    (bc_Constructor*)bc_AtVector(self->Constructors, i);
...
#endif
        bc_CallContext* cctx = bc_NewNameContext(self->Parent->Location,
                                                 self->SuperClass->CoreType);
...
static void create_vtable_interface(bc_Class* self) {
        ...
        for (int i = 0; i < tbl->Length; i++) {
                ...
                for (int j = 0; j < interVT->Elements->Length; j++) {
                        ...
                        bc_Method* classVTM =
                            class_find_impl_method(self, interVTM);
...
static bc_Method* class_find_impl_method(bc_Class* self,
                                         bc_Method* virtualMethod) {
        bc_CallContext* cctx =
            bc_NewNameContext(self->Parent->Location, self->Parent);
        bc_Method* ret = NULL;
