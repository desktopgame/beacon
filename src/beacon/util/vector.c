...
bc_Vector* bc_MallocVector(const char* filename, int lineno) {
        bc_Vector* ret =
            (bc_Vector*)bc_MXMalloc(sizeof(bc_Vector), filename, lineno);
        ret->Length = 0;
        ret->Capacity = 16;
        ret->Memory =
            (bc_VectorItem*)bc_MXMalloc(SLOT_SIZE * 16, filename, lineno);
        return ret;
}
...
int bc_ReserveVector(bc_Vector* self) {
        ...
        bc_VectorItem* temp =
            MEM_REALLOC(self->Memory, SLOT_SIZE * newCapacitySize);
        assert(temp != NULL);
        self->Memory = temp;
...
bool bc_IsEmptyVector(bc_Vector* self) { return self->Length == 0; }
...
void bc_VectorDeleterByFree(bc_VectorItem item) { MEM_FREE(item); }
