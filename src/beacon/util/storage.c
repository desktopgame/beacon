...
static Unit* new_unit(Storage* self, size_t size, const char* filename, int
lineno); static void get_next_storage(Storage* self, size_t size); static void
set_unit_tag(Unit* self, UnitTag tag); static UnitTag get_unit_tag(Unit* self);
...
{ get_next_storage(self, keep_size); return keep(self->Next, keep_size,
filename, lineno);
        }
...
void* re_size(Storage* self, void* area, size_t new_size, const char* filename,
int lineno) { return NULL;
}
