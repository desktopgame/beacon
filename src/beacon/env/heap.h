...
/**
 * ミューテックスによって実行中のスレッドがウェイトに入ると呼ばれます。
 * 複数のスレッドが一つのミューテックスによって同期するとき、
 * 先に待ち行列に入ったスレッドが `CheckSTWRequest`
 * で待機するとデッドロックになってしまいます。
 * その対策として、ロックの前後で`bc_Begin/EndSyncScriptThread`を呼び出します。
 * この関数を呼び出すことによって、アクティブなスレッドの数は一つ減ります。
 * 本来`CheckSTWRequest`では既存のスレッドを全てウェイトさせる必要があるので、
 * 内部のアトミック変数を用いてそれがアクティブなスレッドの数と一致するかを確認しています。
 * なので、同期のためにすでに`Sync`が呼び出されているなら正しくSTWが発生します。
 * ただし、`Sync`より先にウェイトに入った場合やはりデッドロックが発生します。
 * この関数はアクティブなスレッドが減らされた時に呼び出されます。
 * 呼び出された時に先にウェイトに入っていたならSTWを発生させます。
 * @return
 */
