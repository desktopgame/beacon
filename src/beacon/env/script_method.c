...
bc_ScriptMethod* bc_NewScriptMethod() {
        bc_ScriptMethod* ret =
            (bc_ScriptMethod*)MEM_MALLOC(sizeof(bc_ScriptMethod));
        ret->Env = NULL;
        return ret;
}

void bc_ExecuteScriptMethod(bc_ScriptMethod* self, bc_Method* parent,
        ...
        if (!bc_IsStaticModifier(BC_MEMBER_MODIFIER(parent))) {
                ...
                cfr = bc_PushCallFrame(bc_GetScriptThreadContext(),
                                       receiver_obj->GType, aArgs, aTArgs);
        } else {
                cfr = bc_PushCallFrame(bc_GetScriptThreadContext(), NULL, aArgs,
                                       aTArgs);
        }
