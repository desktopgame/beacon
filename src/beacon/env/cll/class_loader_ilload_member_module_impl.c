...
static bc_ILPropertyBody* CLILProperty_body(bc_ClassLoader* self,
...
void CLILMemberList(bc_ClassLoader* self, bc_ILType* current, bc_AST* amember,
        ...
        if (amember->Tag == AST_MEMBER_DECL_LIST_T) {
                for (int i = 0; i < amember->Children->Length; i++) {
                        CLILMemberList(self, current, bc_AtAST(amember, i),
                                       level);
                }
...
void CLILProperty(bc_ClassLoader* self, bc_ILType* current, bc_AST* aprop,
        ...
        ret->Access = level;
        ret->Set =
            CLILProperty_body(self, current, aset, IL_PROPERTY_SET_T, level);
        ret->Get =
            CLILProperty_body(self, current, aget, IL_PROPERTY_GET_T, level);
