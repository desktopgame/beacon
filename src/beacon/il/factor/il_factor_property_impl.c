...
bc_ILPropertyAccess* bc_MallocILPropertyAccess(const char* filename,
                                               int lineno) {
        bc_ILPropertyAccess* ret =
            bc_MXMalloc(sizeof(bc_ILPropertyAccess), filename, lineno);
        ret->Source = NULL;
        ret->Index = -1;
...
bc_GenericType* bc_EvalILProperty(bc_ILPropertyAccess* self, bc_Enviroment* env,
                                  bc_CallContext* cctx) {
