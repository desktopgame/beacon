...
bc_ILVariableStatic* bc_NewILVariableStatic() {
        bc_ILVariableStatic* ret =
            (bc_ILVariableStatic*)MEM_MALLOC(sizeof(bc_ILVariableStatic));
        ret->FQCN = NULL;
        ret->TypeArgs = NULL;
        return ret;
}
...
void bc_LoadILVariableStatic(bc_ILVariableStatic* self, bc_Enviroment* env,
                             bc_CallContext* cctx) {
        //	FATAL("not implemented, because must be wrapped by call
        //operator");
}

bc_GenericType* bc_EvalILVariableStatic(bc_ILVariableStatic* self,
                                        bc_Enviroment* env,
                                        bc_CallContext* cctx) {
        return NULL;
}
