...
bc_ILInferencedTypeInit* bc_NewILInferencedTypeInit(bc_StringView namev) {
        bc_ILInferencedTypeInit* ret = (bc_ILInferencedTypeInit*)MEM_MALLOC(
            sizeof(bc_ILInferencedTypeInit));
        ret->Name = namev;
        ret->Value = NULL;
        return ret;
}

void bc_GenerateILInferencedTypeInit(bc_ILInferencedTypeInit* self,
...
void bc_LoadILInferencedTypeInit(bc_ILInferencedTypeInit* self,
                                 bc_Enviroment* env, bc_CallContext* cctx) {
        //代入するオブジェクトを計算
        ...
        BC_ERROR();
        // voidは代入できない
        if (gtp->CoreType != NULL && gtp->CoreType == BC_TYPE_VOID) {
        ...
        bc_SymbolEntry* e = bc_EntrySymbolTable(env->Symboles, gtp, self->Name);
        self->Symbol = e;
}
